...
class GL_Camera {
public:
    GL_Camera(glm::vec3 position, float fov=60.0f, float nearPlane=0.01f, float farPlane=150.0f, float aspectRatio=1.7778f) : fov(fov), nearPlane(nearPlane), farPlane(farPlane), aspectRatio(aspectRatio), sphericalPosition(glm::vec3(0, 0, 1)), target(position), up(glm::vec3(0,1,0)){ 
